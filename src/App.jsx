...
import { 
  Sprout, ShoppingCart, Home, Leaf, Calendar, 
  ClipboardList, BarChart3, FileSpreadsheet, Sliders, TrendingUp,
  X, ArrowRight, CheckCircle, CalendarDays
} from 'lucide-react';
...
const GUIDES_ONGLETS = {
  marche: {
    ...
    description: "Commencez par indiquer combien de paniers AMAP, ventes au marché et restaurants vous souhaitez fournir chaque semaine.",
    etapes: [
    ...
    conseil: "💡 Commencez prudemment la 1ère année. Vous pourrez toujours augmenter ensuite."
  },
  jardins: {
    titre: "🏡 Configurez vos espaces de culture",
    description: "Décrivez vos jardins : nombre de planches et leur longueur. Cela détermine votre capacité de production.",
    etapes: [
    ...
    conseil: "💡 Une planche standard fait 15m × 80cm. Comptez ~12m² par planche de 15m."
  },
  simulateur: {
    titre: "⚖️ Trouvez l'équilibre offre/demande",
    description: "Le simulateur calcule si votre capacité (planches) peut satisfaire votre demande (marché). Ajustez les curseurs pour trouver le bon équilibre.",
    etapes: [
  ...
  cultures: {
    ...
    description: "Choisissez les légumes à cultiver. Le système calcule automatiquement le nombre de séries et planches nécessaires.",
    etapes: [
    ...
    conseil: "💡 Commencez par 5-8 cultures maîtrisées plutôt que 20 cultures mal gérées."
  },
  planification: {
    titre: "📅 Visualisez votre calendrier",
    description: "Le planning affiche toutes vos séries sur un calendrier. Vérifiez les chevauchements et optimisez vos rotations.",
    etapes: [
    ...
    conseil: "💡 Les couleurs indiquent les phases : semis (vert), croissance (bleu), récolte (orange)."
  },
  taches: {
    titre: "📋 Gérez vos interventions",
    description: "Le calendrier des tâches liste toutes les interventions à réaliser : semis, plantations, récoltes, entretien...",
    etapes: [
    ...
    conseil: "💡 Prévoyez 60% du temps pour la récolte, 30% entretien, 10% implantation."
  },
  agenda: {
    titre: "📆 Vue opérationnelle",
    description: "L'agenda synthétise tout ce qui se passe cette semaine : besoins marché, tâches du jour, état des jardins.",
    etapes: [
  ...
  resultats: {
    ...
    description: "Les résultats comparent votre CA planifié aux coûts (intrants, main d'œuvre) pour évaluer la viabilité économique.",
    etapes: [
  ...
  export: {
    ...
    description: "Téléchargez un fichier Excel complet avec toutes vos données : planning, cultures, résultats financiers...",
    etapes: [
    ...
    conseil: "💡 Exportez régulièrement pour garder un historique de vos planifications."
  }
};
    ...
    <div className="mb-4 md:mb-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl overflow-hidden">
      ...
      <div className="px-3 md:px-4 py-2 md:py-3 bg-blue-100 flex items-center justify-between">
        <h3 className="font-bold text-blue-800 flex items-center text-sm md:text-base">
          ...
          <span className="sm:hidden">{guide.titre.split(' ').slice(0, 3).join(' ')}...</span>
        </h3>
        <button
      ...
      <div className="p-3 md:p-4">
        <p className="text-gray-700 mb-3 md:mb-4 text-sm md:text-base">{guide.description}</p>
        
        {/* Étapes */}
        <div className="bg-white rounded-lg p-2 md:p-3 mb-3">
          <p className="text-xs md:text-sm font-semibold text-gray-600 mb-2">📋 Étapes :</p>
          <ol className="list-decimal list-inside space-y-1 text-xs md:text-sm text-gray-600">
            {guide.etapes.map((etape, i) => (
              <li key={i} className="pl-1 md:pl-2">{etape}</li>
            ))}
          </ol>
        </div>
...
function App() {
  ...
  const nbGuidesMasques = Object.values(guidesVisibles).filter(v => v === false).length;
  ...
  useEffect(() => {
    localStorage.setItem('longueurPlancheDefaut', longueurPlancheDefaut.toString());
  ...
  const totalPaniers = marcheValide.amap + marcheValide.marche + marcheValide.restaurant;
  const coefficientMaturite = NIVEAUX_MATURITE[niveauMaturite]?.coefficient || 0.70;
  ...
  const tabs = [
    { id: 'marche', label: 'Marché', icon: ShoppingCart, description: 'Définissez votre demande' },
    { id: 'jardins', label: 'Jardins', icon: Home, description: 'Configurez vos planches' },
    { id: 'simulateur', label: 'Simulateur', icon: Sliders, description: 'Trouvez l\'équilibre' },
    { id: 'cultures', label: 'Cultures', icon: Leaf, description: 'Planifiez vos séries' },
    { id: 'planification', label: 'Planification', icon: Calendar, description: 'Visualisez le calendrier' },
    { id: 'taches', label: 'Tâches', icon: ClipboardList, description: 'Gérez vos travaux' },
    { id: 'agenda', label: 'Agenda', icon: CalendarDays, description: 'Vue opérationnelle' },
    { id: 'resultats', label: 'Résultats', icon: BarChart3, description: 'Analysez la rentabilité' },
    { id: 'export', label: 'Export', icon: FileSpreadsheet, description: 'Exportez vos données' }
  ];
    ...
    <div className="bg-gradient-to-r from-green-600 to-emerald-600 text-white p-6 relative">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-6">
          {[
            { num: 1, label: 'Marché', desc: 'Vos besoins clients', icon: '🛒' },
            { num: 2, label: 'Jardins', desc: 'Vos parcelles', icon: '🏡' },
            { num: 3, label: 'Simulateur', desc: 'Équilibre offre/demande', icon: '⚖️' },
            { num: 4, label: 'Cultures', desc: 'Sélection légumes', icon: '🌱' },
            { num: 5, label: 'Planification', desc: 'Calendrier visuel', icon: '📅' },
            { num: 6, label: 'Résultats', desc: 'Analyse financière', icon: '📊' }
          ].map((step, i, arr) => (
            <React.Fragment key={step.num}>
              <div className="bg-white/20 backdrop-blur rounded-lg p-3 text-center min-w-[100px]">
                <div className="text-2xl mb-1">{step.icon}</div>
                <div className="font-bold text-sm">{step.num}. {step.label}</div>
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          <button
            onClick={() => { dismissOnboarding(); setActiveTab('marche'); }}
          ...
          <button
            onClick={() => { dismissOnboarding(); setActiveTab('jardins'); }}
    ...
    return (
      <div className="bg-amber-50 border-l-4 border-amber-400 p-4 mb-4">
        <div className="flex items-start">
          ...
          <div>
            <h4 className="font-bold text-amber-800">Configuration incomplète</h4>
            <p className="text-sm text-amber-700 mt-1">
              Pour utiliser le simulateur et planifier vos cultures, vous devez d'abord :
            </p>
            <ul className="mt-2 space-y-1">
              {manqueMarche && (
                <li className="flex items-center text-sm text-amber-700">
                  <span className="w-5 h-5 rounded-full bg-amber-200 flex items-center justify-center mr-2 text-xs">1</span>
                  <button 
              ...
              {manqueJardins && (
                <li className="flex items-center text-sm text-amber-700">
                  <span className="w-5 h-5 rounded-full bg-amber-200 flex items-center justify-center mr-2 text-xs">2</span>
                  <button 
  ...
  return (
    <div className="min-h-screen bg-gray-100">
        ...
        <div className="max-w-7xl mx-auto px-3 md:px-4 py-3 md:py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2 md:space-x-3 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="text-base md:text-xl font-bold text-gray-900 truncate">
                  <span className="hidden sm:inline">Planification Maraîchère Bio-Intensive</span>
            ...
            <div className="flex items-center space-x-2 md:space-x-4 flex-shrink-0">
              ...
              >
                <span className="text-base md:text-lg">{niveauConfig.icone}</span>
                <span className="font-medium text-xs md:text-sm hidden sm:inline" style={{ color: niveauConfig.couleur }}>
                  {niveauConfig.label}
                </span>
                <span 
            ...
            {tabs.map((tab, index) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`w-5 h-5 rounded-full flex-shrink-0 items-center justify-center text-xs font-bold hidden md:flex ${
                    isActive ? 'bg-green-600 text-white' : 
                    isComplete ? 'bg-green-400 text-white' : 'bg-gray-300 text-gray-600'
                  }`}>
                    {isComplete && !isActive ? <CheckCircle className="w-3 h-3" /> : index + 1}
                  </span>
      ...
      <main className="max-w-7xl mx-auto px-3 md:px-4 py-4 md:py-6">
        ...
        {!configurationComplete && activeTab !== 'marche' && activeTab !== 'jardins' && (
          <ConfigurationAlert />
        )}
        ...
        {activeTab === 'jardins' && (
          <ConfigurationJardins
            jardins={jardins}
            setJardins={setJardins}
          />
        )}
      ...
      <footer className="bg-white border-t border-gray-200 mt-8">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between text-sm text-gray-500">
            <p>
              Méthodologie inspirée de l'Institut Jardinier Maraîcher • 
              Adapté pour le Sud-Ouest de la France
            </p>
            <div className="flex items-center space-x-4">
              {configurationComplete ? (
                <>
                  ...
                  <span>
                    {totalPlanches} planches disponibles
                  </span>
                </>
              ) : (
