...
const charteTaches = {
  ...
  "Bettes à carde": {
    couvreSol: "Couvre-sol tissé + filet anti-insecte",
    fauxSemis: null,
    pyrodesherbeur: null,
    herse1: null,
    herse2: null,
  ...
  "Bok choy": {
    couvreSol: "Filet anti-insecte ou couverture flottante",
    fauxSemis: null,
    pyrodesherbeur: null,
    herse1: null,
    herse2: null,
  ...
  Frisées: {
    couvreSol: "Avec ou sans couvre-sol tissé",
    fauxSemis: null,
    pyrodesherbeur: null,
    herse1: 14,
    herse2: 21,
  ...
  "Haricots grimpants": {
    couvreSol: "Couvre-sol tissé + tuteurage",
    fauxSemis: null,
    pyrodesherbeur: null,
    herse1: null,
    herse2: null,
  ...
  Oignons: {
    couvreSol: "Peut être planté sur couvre-sol",
    fauxSemis: -12,
    pyrodesherbeur: 0,
    herse1: null,
    herse2: null,
...
const tempsTaches = {
  fauxSemis: {
    nom: "Faux semis",
    temps: 5,
    categorie: "Préparation sol",
    icon: "🌱",
  },
  pyrodesherbeur: {
    nom: "Pyrodésherbeur",
    temps: 3,
    categorie: "Désherbage",
    icon: "🔥",
  },
  herse1: {
    nom: "Herse étrille 1",
  ...
  paillage: {
    nom: "Paillage/épandage",
    temps: 20,
    categorie: "Protection",
    icon: "🌾",
  },
  pincer: {
    nom: "Pincer/couper têtes",
    temps: 10,
    categorie: "Taille",
    icon: "✂️",
  },
  tondre: {
    nom: "Tondre et bâcher",
    temps: 15,
    categorie: "Fin culture",
    icon: "🏁",
  },
};
...
const CalendrierTaches = ({
  ...
  const totalPlanches = culturesSelectionnees.reduce(
  ...
  culturesSelectionnees.forEach((culture) => {
    ...
    tempsStandardsTotal.transplant +=
      tempsStandards.transplant.temps * nbPlanches;
    tempsStandardsTotal.recolte +=
      tempsStandards.recolte.temps * nbPlanches * 4; // ~4 récoltes
    tempsStandardsTotal.nettoyage +=
      tempsStandards.nettoyage.temps * nbPlanches;
    tempsStandardsTotal.arrosage +=
      tempsStandards.arrosage.temps * nbPlanches * 20; // ~20 arrosages
  });

  const tempsStandardsTotalMin = Object.values(tempsStandardsTotal).reduce(
    (a, b) => a + b,
    0
  );
  const tempsTotalGlobal = tempsTotal + tempsStandardsTotalMin;

  // 🆕 V26 : Exporter les heures vers App.jsx
  useEffect(() => {
    if (setHeuresTaches) {
      setHeuresTaches(tempsTotalGlobal / 60); // Convertir minutes en heures
    }
  }, [tempsTotalGlobal, setHeuresTaches]);
    ...
    return (
      <div className="bg-white rounded-lg shadow-md border border-gray-200 p-8 text-center">
        ...
        <p className="text-gray-500">
          Sélectionnez des cultures dans l'onglet "Cultures" pour voir le
          calendrier des tâches.
        </p>
      </div>
    );
      ...
      <div className="bg-gradient-to-r from-teal-50 via-cyan-50 to-sky-50 rounded-xl shadow-lg border-2 border-teal-200 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm border border-purple-100 text-center">
            ...
            <div className="text-2xl font-bold text-purple-600">
              {Math.ceil(tempsTotalGlobal / 60 / 8)}
            </div>
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm border border-amber-100 text-center">
            ...
            <div className="text-2xl font-bold text-amber-600">
              {culturesSelectionnees.reduce(
                (sum, c) => sum + getTachesCulture(c).length,
                0
              )}
            </div>
        ...
        <div className="bg-white rounded-lg p-3 border">
          <h4 className="text-sm font-semibold text-gray-700 mb-2">
            ⚙️ Tâches spécifiques par catégorie
          </h4>
          <div className="flex flex-wrap gap-2">
            {Object.entries(tempsParCategorie)
              .filter(([_, t]) => t > 0)
              .map(([cat, temps]) => (
                <span
                  key={cat}
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-200 p-6">
        ...
        <div className="space-y-3">
          {culturesSelectionnees.map((culture) => {
            ...
            const tempsEstime = taches.reduce(
              (sum, t) => sum + t.temps * (culture.totalPlanches || 1),
              0
            );
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {culture.totalPlanches} pl. • {taches.length} tâches •{" "}
                        {formatTemps(tempsEstime)}
                      </p>
                    </div>
                  </div>
                    ...
                    {taches.length > 0 ? (
                          ...
                          {taches.map((tache, idx) => (
                            <div
                              ...
                              className={`flex items-center p-3 rounded-lg border ${
                                tache.categorie === "Désherbage"
                                  ? "bg-orange-50 border-orange-200"
                                  : tache.categorie === "Fertilisation"
                                  ? "bg-blue-50 border-blue-200"
                                  : tache.categorie === "Protection"
                                  ? "bg-green-50 border-green-200"
                                  : tache.categorie === "Taille"
                            ...
                            >
                              ...
                              <div className="flex-1 ml-3">
                                ...
                                <div className="text-xs text-gray-500 mt-1">
                                  {tache.categorie} • ~
                                  {tache.temps * (culture.totalPlanches || 1)}{" "}
                                  min ({tache.temps} min/pl.)
                                </div>
                              </div>
                            </div>
                          ))}
                      ...
                      <div className="mt-4 bg-gray-50 p-4 rounded-lg text-center">
                        <p className="text-gray-500 text-sm">
                          ✨ Aucune tâche spécifique - Culture à faible
                          entretien
                        </p>
                      </div>
                    )}
                    ...
                    <div className="mt-4 bg-blue-50 p-3 rounded-lg border border-blue-200">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 text-xs">
                        ...
                        <div className="bg-white p-1.5 sm:p-2 rounded text-center">
                          ...
                          <div className="font-bold text-xs sm:text-sm">
                            {formatTemps(
                              tempsStandards.transplant.temps *
                                (culture.totalPlanches || 1)
                            )}
                          </div>
                        </div>
                        <div className="bg-white p-1.5 sm:p-2 rounded text-center">
                        ...
                        <div className="bg-white p-1.5 sm:p-2 rounded text-center">
                          ...
                          <div className="font-bold text-xs sm:text-sm">
                            {formatTemps(
                              tempsStandards.nettoyage.temps *
                                (culture.totalPlanches || 1)
                            )}
                          </div>
                        </div>
