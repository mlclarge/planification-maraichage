...
// 📱 V22 : Optimisations mobile - blocs repliés + bouton rouge
...
const ConfigurationJardins = ({ jardins, setJardins }) => {
  ...
  // Accordéons des jardins individuels
  const [accordeonsOuverts, setAccordeonsOuverts] = useState({});
  
  // 📱 Accordéons des sections principales (fermés par défaut sur mobile)
  ...
  // 📱 Fermer les sections sur mobile au premier chargement
  useEffect(() => {
    if (isMobile) {
      setSectionsOuvertes({
        configuration: false,
  ...
  const ajouterJardin = () => {
    ...
    // 📱 Ouvrir la section jardins si elle est fermée
    setSectionsOuvertes(prev => ({ ...prev, jardins: true }));
  };
  ...
  // 📱 Composant Section avec accordéon
  const Section = ({ id, title, icon, children, badge = null }) => {
    const isOpen = sectionsOuvertes[id];
      ...
      <div className="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden">
        ...
        >
          ...
          <div className="flex items-center space-x-2">
            {badge && <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded font-medium">{badge}</span>}
            <ChevronDown className={`w-6 h-6 text-gray-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
          </div>
        </button>
        {isOpen && <div className="px-4 pb-4 sm:px-6 sm:pb-6 border-t">{children}</div>}
      </div>
    );
  };
  ...
  return (
    <div className="space-y-6">
      {/* 📱 Section Configuration des Jardins */}
      <Section id="configuration" title="Configuration des Jardins" icon="🏡" badge={`${totalPlanches} planches`}>
        <div className="pt-4">
          {/* 📱 Bouton Ajouter - Rouge et petit sur mobile, Vert et grand sur desktop */}
          <div className="flex justify-end mb-6">
            {/* Mobile : bouton rouge compact */}
            <button 
              onClick={ajouterJardin} 
              className="sm:hidden flex items-center space-x-1 px-3 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 shadow transition-all text-sm"
            >
              <Plus className="w-4 h-4" />
              <span className="font-medium">Ajouter</span>
            </button>
            {/* Desktop : bouton vert standard */}
            <button 
              onClick={ajouterJardin} 
              className="hidden sm:flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg hover:from-green-700 hover:to-green-800 shadow-md transition-all"
            >
      ...
      {/* 📱 Section Vos Jardins */}
      {jardins.length > 0 && (
        <Section id="jardins" title="Vos Jardins" icon="🌿" badge={`${jardins.length}`}>
          <div className="pt-4">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              ...
              <button 
                onClick={ajouterJardin}
                className="sm:hidden w-full py-2 border-2 border-dashed border-red-300 rounded-lg text-red-600 hover:border-red-400 hover:bg-red-50 transition-all flex items-center justify-center font-medium text-sm"
              >
              ...
              <button 
                onClick={ajouterJardin}
                className="hidden sm:flex w-full py-3 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-green-400 hover:text-green-600 hover:bg-green-50 transition-all items-center justify-center font-medium"
              >
                <Plus className="w-5 h-5 mr-2" />
      ...
      {/* 📱 Section Paliers de Performance */}
      {jardins.length > 0 && (
        <Section id="paliers" title="Paliers de Performance" icon="📊">
          <div className="pt-4">
